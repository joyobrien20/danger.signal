...
growth.modGomp <- function(input = " ", output.name = " ", skip = "", 
  ...
  colnames(results) <- c("Curve", "b0", "b0.se", "A", "A.se", "umax", "umax.se", "L", "L.se", "RSME", "CV", "outlier")
  results <- as.data.frame(results)

  # Initialize Output
  outfile <- paste(output.dir, paste(output.name, ".txt", sep=""), sep ="/")
  titles <- c("Curve", "b0", "b0.se", "A", "A.se", "umax", "umax.se", "L", "L.se","RSME", "CV", "outlier")
  ...
  for(i in 1:length(samples)){
    ...
    #best.f1<<-fit1$res.mod[[best]] # Schoolmaster's suggestion to make best.fit globally available
 ...
 #   abline(h = 0, lty = 3, lwd = 2)
    ...
    cfs<-coef(best.f1)
    residuals<- s-m.gomp(t,coef(best.f1))
    results$Curve[i] <- samples[i]
    results$b0[i]<-round(cfs['b0'], 4)
    results$b0.se[i]<- round(sqrt(diag(vcov(best.f1)))[1],5)
    results$A[i]<-round(cfs['A'], 4)
    results$A.se[i]<- round(sqrt(diag(vcov(best.f1)))[2],5)
    results$umax[i]<-round(cfs['umax'], 4)
    results$umax.se[i]<- round(sqrt(diag(vcov(best.f1)))[3],5)
    results$L[i]<-round(cfs['L'] + tmpdata$t[1], 4)
    results$L.se[i]<- round(sqrt(diag(vcov(best.f1)))[4],5)
    #results$z[i]<-round(cfs['z'], 4)
    #results$z.se[i]<- round(sqrt(diag(vcov(best.f1)))[5],5)
    results$RSME[i] <- round(exp(cfs['z']),4)
    results$CV[i] <- round( ((exp(cfs['z']))/ mean(realdata$s) *100), 3)
    results$outlier[i] <- sum(abs(residuals-mean(residuals))/sd(residuals)>3)
