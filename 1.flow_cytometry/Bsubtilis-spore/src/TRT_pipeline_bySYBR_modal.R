...
sample.var <- c("sample.day.hr","exp.plate","t.exp","dilution",
                "trt","bio.rep","well","rep","xt","num") 
...
complot <- 
  ...
  geom_density2d(col = "black",  size = 0.3, alpha = 0.5) +
  scale_alpha_continuous(guide = FALSE) +
  facet_wrap(~well)+
  theme_cowplot()+
  panel_border()+
...
ggsave2(filename = paste0("scatterSYBR_",fcsset[[i]]@description$`$SRC`,".png" ), 
        plot = complot,
        path = here("fig/gate_plots/", day), scale = 3)
