#analysis of inducible sigma factors experiment using Karava's analysis.R as reference.
rm(list=ls())
setwd("C:/Users/danschw/Box/Lab/research/Bacillus phage/ADGs/sigF/inducible_sigF/OD_FCM2/")
source("src/functions.R")
source("src/DS_functions.R")
set.seed(1)
#choose data
folders <- 
list.dirs("data/FCM/fcs/",  full.names = T)

###sort folders to have noIPTG first
#  This is for building cluster prediction models based on controls

# remove folder of folders
folders <- folders[-1]
#In this run
# place "xIPTG" first"
...
# sample.var <- c("strain","colony","media","treat","dilution","well" ,"rep","xt","num") 
...
#save plot
ggsave2(paste0("fig/gate_plots/singlet_",fcsset[[i]]@description$`$SRC`,".pdf" ), plot_grid(plotlist = plot.list))
...
ggsave2(paste0("fig/gate_plots/noise_",fcsset[[i]]@description$`$SRC`,".pdf"), noise.plot)
...
#### predict centers of sub-populations ####
         ...
         # # save(df.mix, file="data/TS01_cluster_model.Rdata")
         # 
         # Load reference containing all subpopulations
         # I pre-compiled a model using no phage triplicate in delta6_DSM_T4_x10
         # load("data/TS01_cluster_model.Rdata")
...
current.strain <- df.set$strain[1]
...
write_csv(df.stats,paste0("data/output/",fcsset[[i]]@description$`$SRC`,".csv"))
